...
if($_GPC['uid'] || $_SESSION['resumeuid']){
	if($_GPC['uid']){
		$member = m('member')->get_member($_GPC['uid']);
		$resume = m("resume")->get_resume($_GPC['uid']);
	}
   
   if($_SESSION['resumeuid']){
		$member = m('member')->get_member($_SESSION['resumeuid']);
		$resume = m("resume")->get_resume($_SESSION['resumeuid']);
		unset($_SESSION['resumeuid']);
   }
